/***** バージョン表示（デバッグ用） *****/
console.warn('[himegoto] app.js v-expiry-3.1');

/***** 設定（あなたの最新GAS URLに更新して使ってください） *****/
...
async function callGas(action, payload={}, {retries=0,delay=300}={}){
  ...
  const body=JSON.stringify({action, ...payload}); // Content-Type 付けない
...
function render(){
  ...
  s.customers.forEach((nm,idx)=>{
    ...
    sel.className='btn btn-ghost'; sel.type='button';
    sel.textContent=(s.selected===nm)?'選択中':'選ぶ';
    if(s.selected===nm){ sel.style.background='#2ecc71'; sel.style.color='#fff'; }
    sel.onclick=()=>{ s.selected=nm; setState(s); };

    const del=document.createElement('button');
    del.className='btn btn-danger'; del.type='button';
    del.textContent='削除';
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(btnVerify){
    ...
    btnVerify.addEventListener('click', async ()=>{
      ...
      await new Promise(r=>setTimeout(r,0)); // 表示を確実に
  ...
  if(btnRecalc){
    ...
    btnRecalc.onclick = async ()=>{
      try{
        ...
        if (Number.isFinite(Number(data?.remainingDays))){
          const d=new Date(); d.setHours(0,0,0,0); d.setDate(d.getDate()+Number(data.remainingDays||0));
          setLocalExpiry( ymdFromDate(d) );
        }
      }catch{}
  ...
  if(btnCopy){
    ...
    btnCopy.onclick=()=>{
      ...
      (async ()=>{ try{ await navigator.clipboard.writeText(text); alert('コピーしました'); }catch{ alert('コピーに失敗しました'); } })();
    };
  }
