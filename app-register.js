...
// メッセージ表示
  ...
  function setupMyReferralSection(uid) {
    ...
    // 紹介コードの登録（Functions側で登録）
    try {
      const ensureRefCode = fn.httpsCallable('ensureRefCode');
      ensureRefCode({});
    } catch(e) {}
  ...
  async function handleRegistrationAndReferral(uid, appliedRefCode) {
    ...
    try {
      ...
      await applyReferral({ refCode: appliedRefCode });
    } catch(e) {
      const msg = (e && e.message) ? e.message : String(e);
      logError(`紹介反映に失敗しました: ${msg}`);
    }
  }
